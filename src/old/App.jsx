...
// --- MOTOR DE OSNAP (Referencia a Objetos) ---
const extractSegments = (el) => {
  let segs = [];
  if (el.type === 'line') segs.push([ {x:el.x1, y:el.y1}, {x:el.x2, y:el.y2} ]);
  if (el.type === 'rect') {
     segs.push([ {x:el.x1, y:el.y1}, {x:el.x2, y:el.y1} ]);
     segs.push([ {x:el.x2, y:el.y1}, {x:el.x2, y:el.y2} ]);
     segs.push([ {x:el.x2, y:el.y2}, {x:el.x1, y:el.y2} ]);
     segs.push([ {x:el.x1, y:el.y2}, {x:el.x1, y:el.y1} ]);
  }
  if (el.type === 'polyline' || el.type === 'hatch' || el.type === 'pencil') {
     if(el.points) {
         for(let i=0; i<el.points.length-1; i++) {
           segs.push([el.points[i], el.points[i+1]]);
         }
         if (el.type === 'hatch' && el.points.length > 2) {
           segs.push([el.points[el.points.length-1], el.points[0]]);
         }
     }
  }
  return segs;
...
const findOsnap = (worldPos, elements, threshold, activeStart, excludeId, layers) => {
   let candidates = [];
   let nearbySegments = [];
   ...
   elements.forEach(el => {
      ...
      if (el.type === 'line') {
         ...
         let sa = el.startAngle;
         let ea = el.endAngle;
         if (ea < sa) ea += 2*Math.PI;
         let midA = (sa + ea) / 2;
         candidates.push({ x: el.x + el.r*Math.cos(midA), y: el.y + el.r*Math.sin(midA), type: 'midpoint' });
...
export default function App() {
  ...
  const handleMouseDown = (e) => {
    ...
    if (osnapEnabled && tool !== 'pan' && tool !== 'select' && tool !== 'make_block' && action !== 'panning') {
        const snap = findOsnap(worldPos, elements, 15 / camera.zoom, activeStart, excludeId, layers);
  ...
  const handleMouseMove = (e) => {
    ...
    if (osnapEnabled && tool !== 'pan' && action !== 'panning') {
        const snap = findOsnap(worldPos, elements, 15 / camera.zoom, activeStart, excludeId, layers);
        if (snap) { worldPos = { x: snap.x, y: snap.y }; setActiveSnap(snap); }
